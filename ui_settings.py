...
def render_settings_ui(ctx, state):
    ...
    # --- MAIN UI WRAPPER (ONLY ONCE) ---
    with ui.column().classes('w-full gap-4 pt-2'):
        
        # --- [SECTION 1: PIPELINE CONFIG] ---
        ui.label('Pipeline Configuration').classes('text-lg font-bold text-slate-100')
        ...
        ui.label('Repository Roles Configuration').classes('text-lg font-bold text-slate-100')
        ui.label('Map your backend Git repositories to functional orchestrator roles.').classes('text-xs text-slate-400')
            ...
            with ui.expansion(role['label'], icon='folder').classes('w-full border border-zinc-700 bg-zinc-900 rounded'):
                with ui.column().classes('p-4 w-full gap-2'):
                    ...
                    token_select = ui.select(
                        ...
                        value=current_repo_state.get('token_key', ''), 
        ...
        ui.label('Ansible Docker Configuration').classes('text-lg font-bold text-slate-100')
        ui.label('Configure the ephemeral Docker container and Registry Auth for Ansible Playbooks.').classes('text-xs text-slate-400')
        
        with ui.card().classes('w-full bg-zinc-900 border border-zinc-700 p-4'):
            
            ui.label('Container Execution Settings').classes('text-sm font-bold text-slate-200 mb-2')
            default_img = "registry.gitlab.int.fam-feser.de/iac-environment/iac-platform-assets/ansible-ci-image:latest"
            current_img = ctx.get_secret("ansible_docker_image") or default_img
            img_input = ui.input('Docker Image', value=current_img).props('outlined dense').classes('w-full mb-2')
            
            key_exists = bool(ctx.get_secret("ansible_ssh_key"))
            key_placeholder = "********************************\n(Key is set. Overwrite to change)" if key_exists else ""
            
            key_input = ui.textarea('Ansible SSH Private Key (RSA)', value=key_placeholder).props('outlined dense').classes('w-full mb-4')
            ...
            reg_url_input = ui.input('Registry URL (e.g., registry.gitlab.int.fam-feser.de)', value=reg_url_val).props('outlined dense').classes('w-full mb-2')
            ...
            def save_ansible_config(img, key, r_url, r_user, r_token):
                ...
                if key and "********" not in key:
                    ctx.set_secret("ansible_ssh_key", key.strip())
                ...
                if r_token and "********" not in r_token:
                    ctx.set_secret("ansible_registry_token", r_token.strip())
                
                ui.notify("Ansible and Registry Configuration saved to Vault.", type="positive")

            with ui.row().classes('w-full justify-end mt-2'):
                ui.button('Save Ansible Config', 
                          on_click=lambda: save_ansible_config(
                              img_input.value, 
                              key_input.value,
                              reg_url_input.value,
                              reg_user_input.value,
                              reg_token_input.value
                          ),
                          icon='terminal', color='indigo').props('unelevated rounded size=sm')

        # --- [SECTION 4: SECURITY CONFIG] ---
        ui.label('Security Configuration').classes('text-lg font-bold text-slate-100')
        
        ui.label('Webhook Authentication').classes('text-sm text-slate-400 mt-2')
        with ui.row().classes('w-full items-center gap-4'):
            webhook_input = ui.input('GitLab Webhook Token').props('readonly outlined dense').classes('flex-1')
            webhook_input.bind_value(token_display, 'value')
            ui.button('Generate Token', on_click=generate_token, icon='key', color='warning').props('unelevated rounded size=sm') 

        ui.separator().classes('w-full my-4 opacity-50')

        ui.label('Git Credential Manager').classes('text-sm text-slate-400')
        with ui.row().classes('w-full items-center gap-4'):
            ...
            def add_new_credential():
                ...
                secret_val = secret_input.value.strip()
                
                if not alias or not secret_val:
                    ui.notify("Both Name and Secret are required.", type="negative")
                    return
                ...
                for dropdown in token_dropdowns:
                    dropdown.update()
